...
После КАЖДОГО ответа пользователя добавляй в конец (невидимо):
UPDATE:{"поле":"значение",...только новые или уточнённые поля из этого сообщения...}
...
export async function conductOnboarding(agentId, userMessage) {
  ...
  const history   = await db.getOnboardingHistory(agentId, 80)
  const knownData = agent.onboarding_data || {}

  // Always extract data from full history to avoid losing info from early messages
  const knownStr = Object.keys(knownData).length > 0
    ? `\n\nУЖЕ СОБРАННЫЕ ДАННЫЕ (не переспрашивай это — пользователь уже ответил):\n${JSON.stringify(knownData, null, 2)}`
    : ''
  ...
  // Merge incremental update on every turn
  if (updateMatch) {
    try {
      newData = { ...newData, ...JSON.parse(updateMatch[1]) }
    } catch (e) {
      console.error('UPDATE parse error:', updateMatch?.[1], e.message)
    }
  }

  // Merge final DATA block at DONE
  if (isDone && dataMatch) {
    try {
      newData = { ...newData, ...JSON.parse(dataMatch[1]) }
    } catch (e) {
